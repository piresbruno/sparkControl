...
### Added
- **Live download speed next to progress** — running downloads already showed a per-model progress bar and `downloading · NN%` (parsed from the huggingface tqdm frames in the job log, refreshed every 4 s); now the transfer rate is parsed from the same frames and shown everywhere progress is: model row overlay (`downloading · 45% · 97.4MB/s`), the detail card's progress line, the node Jobs strip, and the Models tab's running-job line. The one-line log preview also shows the FRESHEST tqdm frame now — the bars overwrite one line with `\r`, so the old `\n`-split displayed the oldest frame of the tail.
- **Model filter on the NAS catalog tables** — both the Models tab and the NAS node's Models channel gain a search box ("Filter by name or repository…"): client-side substring match over name + repository, case-insensitive, with a "No model matches" empty state; the pager follows the filtered list while a selected model's detail card stays put. 125-model stores were unscannable at 10 rows/page.
- **Script-class unified into the Serve section** (plan R4 deferred item, now implemented): the node-page "Serve script" launch/log panels are **removed** — script-class runs (library + path scripts, `/api/serving/*` lane) are now controlled cluster-wide from /serve beside recipe deployments. New `GET /api/serve/scripts` fans ONE status-all probe per compute node (running rows + stopped rows for recorded path runs, owner-node aware); launch ports persist to `config/serving/run-ports.json` (`<node>:<scriptId>` → port, dropped on confirmed stop), and `path-scripts.json` records become `{path, port, sparkId}` (legacy strings still read). `/api/serving/start` gains `force` (skips the placement gate, recipe parity) and returns `blocked:true` on the 409 — the Serve page's launch form shares the SAME placement-blocked dialog + force escape as recipes; "▶ start" on a stopped row pre-fills node + path. The Serve table renders script rows (kind chip, port, endpoints + key chip while running, armed stop via the legacy route, expanding 5 s log tail). Node page keeps engine bays, WS truth, the armed hero stop, and Daily trend; hero "Serve new model…" + the CH·02 `Serve ▸` deep-link to /serve. Dropped the now-orphan `listServingScripts` client fn + `ServingScript` type. Live-proven on spark-1: launch by path → cluster row running:8123 → stop → stopped row stays addressable → fixture cleanup leaves `runs: []`.
- **Serve section — cluster model serving with recipes** (branch `docs/serve-recipes-plan`, PR #8; plan: `tasks/SERVE-plan.md`). New nav section controls serving **cluster-wide** over user-owned **recipe folders** on nodes (`start.sh` + `.env`, e.g. the MiaAI GLM recipes), registered by `(node, path)`; the dashboard never edits them. **CH·01 Deployments**: joined state machine per row (`starting/healthy/orphan/foreign/up/stopping/failed`, drift chips for `git HEAD` moves), start/stop/restart/`logs` via the recipe's own dispatch verbs through **detached driver jobs** (survive dashboard restart, re-attach semantics, TERM-first stop), DIRECT/PROXY endpoint cards with the API-key chip. **CH·02 Recipes**: register/scan/re-probe/unregister; probe surfaces entry variants, PORT, MODEL, container sets, `.env` secret *presence only* (values never leave the node). **Placement**: absent-model starts hard-block with a NAS-sync / peer-push remediation plan (jobs run through the existing modelctl job lane); unknown/uncertain inventories only warn; the blocked dialog shows the **capacity math** (free vs needed bytes, minus new `modelctl.reserveFreeGiB` setting) and transfer-contention warnings — warnings never block; `force` escapes and still returns the warnings on the 202. **Multi-node**: `NNODES`/`WORKER_IP` recipes refuse start on worker mismatch with an exact-delta dialog. **Gateway**: `GET/POST /llm/cluster/<servedName>/v1/...` round-robins healthy-first across running deployments by `SERVED_MODEL_NAME` (model basename fallback), 404 with the live served-name list, full trace capture — this also fixed a **pre-existing bug** where port-scoped proxy traces never recorded (`/llm/<spark>/<port>` routes didn't set the per-port capture flag). Upstream proposal filed: modelctl#7 (`ROOT/recipes/NAME/` co-distribution + `cluster inventory --json`).
...
- **Instrument-console node detail page** — Spark pages are rebuilt as a four-channel console (Resources / Serving / Tests / Node) with a data-plate rack header, status-LED channel rail, derived engine-activity pill, direct + proxy endpoint cards, 12-stat serving readout, and armed stop; legacy expert panels remain behind disclosures. Workers navigate to their head.
...
- **Overview card engine status** — head and standalone Overview cards show the same derived engine activity as the Serving console pill: a `Status` stat in the card's stats row (`decoding · 2 run · 1 wait` — activity label, running and waiting request counts; backends without queue depth fall back to `slotsActive` and always report 0 waiting), success tone while serving, warning tone for the "gpu busy · engine silent" blind spot, same 5 s poll-gap hold. Worker cards keep cluster attribution only.
- **Store-root storage visibility** — the NAS store root (`spark.nasRoot`, e.g. `/mnt/llms`) is usually a plain directory of a larger filesystem or a network mount, so it never appeared as its own `lsblk`/`df -l` line and the NAS cards showed `— / —` for store space. Both storage collectors now stat the root directly (host `statfs`, remote second `df` without `-l`) and surface its containing filesystem under the root path label; overview NAS cards also fall back to the global `settings.modelctl.nasRoot` when the node has no own path (same resolution the server uses).
...
- **NAS host node picker** — Settings now has a "NAS host node" dropdown (all sparks, with a modelctl-off hint and an "(auto: head → local → sole spark)" default). It writes `modelctl.nasHostSparkId` through `PUT /api/settings` (deep merge; empty selection clears to auto), replacing the curl workaround for pointing NAS operations (download / sync / catalog / delete) at a specific node.
- **NAS model-store node** — new Spark `kind: "nas"`: a node (typically the Ubuntu VM that mounts the model store) that *owns* the modelctl NAS root instead of serving models. Add/Edit Spark's Unit type gains **NAS model store**: compute-only fields (LLM ports, role/worker, CX7, monitoring toggles) hide, **NAS LLM models path** becomes required (`spark.nasRoot`; empty falls back to the global `modelctl.nasRoot`) and becomes `--root` for every store command, and modelctl integration is forced on. `defaultNasSpark()` prefers a `kind: "nas"` spark before the old head → local → sole chain. The node page becomes a dedicated NAS console (per the approved mockup): store-capacity gauge + `catalog.json` block (generation + refresh job), `modelctl doctor --json` audit with two-click `repair-active --apply`, master-detail model list (resolved path, copyable serve command, `RUN.md` excerpt; `update` / `sync-cards` / dry-run delete-plan preview + two-click delete), single **and** queued HF downloads, modelctl install-version vs latest GitHub release, and this node's job history. Overview shows a NAS card (store gauge, model count, modelctl chip, running job) with no VRAM/GPU bars. New job kinds: `queue`, `catalog-refresh`, `repair-active`, `cleanup-quarantine`, `sync-cards`, `update` — the queue's `downloads.yaml` is built server-side from validated entries (raw YAML is never accepted). New cached read endpoints: `GET /api/modelctl/release`, `/doctor`, `/catalog`, `/nas/:model/detail`, `/nas/:model/delete-plan`; per-node `test` skips the LLM/Comfy probes for NAS nodes.

- **Prefill bench running chip (Tests channel)** — an in-flight prefill benchmark is reported on the TST·03 card of the node detail page: level progress, the size being measured with a live clock for it, and the throughput of the last finished level (`running · 1/2 · 256k · 42s · 745 tok/s`); hovering shows the in-flight line with its abort ceiling plus one line per measured level (`Prefilling 256k for 42s, cap 36m` / `4k · 745.3 tok/s · TTFT 1.20s`). The action becomes **Open prefill bench**, since a second concurrent run is rejected with 409. Prefill is not streamed, so elapsed time — not a percentage — is the finest intra-level signal that exists; the job now stamps `progress.levelStartedAt` / `timeoutMs` for it. Poll-based (1 s while running, 10 s idle).
- **Engine "busy but silent" feedback** — the Serving activity pill now carries how long the engine has been busy (`processing · 2m 14s · 2 req · 0.0 tok/s`) and the generation dial reports how long the output counter has been flat (`total 6 586 out tok · no output 2m 14s`). A long prefill reports no rate until the request finishes — vLLM counts prompt tokens per completed request — so these clocks (`busySinceAt` / `lastOutputAt`, tracked centrally in the probe for every backend) are what separate "working" from "wedged".
- **In-flight LLM request visibility & cancellation (A4)** — every proxied LLM request is tracked live (`server/proxy/inflightRegistry.js`) with spark, port, model, stream flag, client identity (IP, user-agent, stable 12-hex `clientId`), streamed-token estimate and TTFT. The Analysis page gains a **Live & Clients** panel (2 s poll): per-request Cancel, per-client Flush, and a global **Stop all** (`POST /api/llm/stop-all` returns per-source counts for proxy / decode bench / prefill bench / showcase). Browser-side aborts now destroy the upstream socket, so streaming generations stop server-side (the incident-proven mechanism); cancels are recorded as first-class traces (`error: "cancelled (user | client disconnect | stop-all | flush client)"`), and an optional per-spark/port concurrency cap (`proxyMaxInflightPerPort`, default 8, 0 = off) answers 429 before the upstream is touched. Non-streaming caveat (no vLLM abort API): the engine keeps decoding to completion — the client sees a 502 only before the first response byte, a truncated stream after.
- **Trace rev 2: full-fidelity capture & deep analysis** — traces gain `clientIp` / `clientUa` / `clientId` / `toolsReq` / `toolsUsed` (aggregated from OpenAI `tool_calls` and Anthropic `tool_use` events) / `cachedTokens` (engine-reported cache hits) / `bodyTruncated` via an in-place `PRAGMA table_info` migration (old `traces.sqlite` files upgrade on boot, rows intact). Body caps move from fixed 32 KiB / 64 KiB to `traceMaxReqBody` / `traceMaxResBody` knobs (4 MiB defaults, 16 MiB ceiling) so ~100k-token prompts are captured whole; `traceRetentionDays` (default 7) replaces the fixed retention. New endpoints: `GET /api/traces/stats` (totals plus by client / model / path / tool and hourly buckets), `GET /api/llm/clients` (live clients with in-flight lists), `DELETE /api/llm/clients/:clientId` (flush one client), and `clientId` + `q` (SQL LIKE body search, no FTS5) filters on the trace list. The Analysis page gains a **Summary** tab (1h/24h/7d cards, four breakdown tables, hourly histogram, click-through to a client's traces), Client/Tools columns, the new fields in the detail modal, a debounced search box, and inline client labels (`clientLabels` setting).
- **Trace rev 3: client hostname** — proxied requests additionally record `clientHost`, resolved from the client IP via reverse DNS (`server/proxy/clientHost.js`: `dns.promises.reverse`, 10 min positive *and* negative TTL, one in-flight lookup shared by concurrent callers, 750 ms cap so a slow resolver never delays a record, injectable resolver for tests). In-place `PRAGMA table_info` migration again — existing `traces.sqlite` files gain the column with rows intact. Analysis now *shows* the identity that was already stored: the Live & Clients panel row prints `10.0.30.173 · box.local`, the trace-table client cell carries it as its tooltip, and the detail sheet's Timing tab gains **Client IP / Hostname / User agent** rows. `GET /api/llm/clients` resolves hostnames per unique IP (cache makes repeat polls free). LANs without PTR records simply show the bare IP.
- **Serve an arbitrary script path on the node** — the Serve script panel (CH·02) accepts a path to a file **on the target node** (e.g. `/home/me/start-vllm.sh`) in addition to a `config/serving/` library script; a non-empty path takes precedence and disables the picker. Path runs get a deterministic id (`derivePathScriptId`: sanitized basename + 6-hex sha1 of the absolute path — always suffixed, so it can never collide with a library id) persisted to `config/serving/path-scripts.json`, so Stop / Status / Log and the running-script probe keep working across restarts. The node-local file is checked at launch (`__NO_SCRIPT__` → "script not found on node — check the path") and the env/pidfile/log contract is identical to library scripts.
- **Fleet Energy per-node breakdown** — `/api/fleet-energy` gains `nodeEnergy24hKwh` / `nodeEnergy31dKwh` (the tracker already integrated per-node Wh and merely was not returning it); null per node without coverage, all-null while membership is changed, matching the existing hourly-series guard. The Overview card lists **Last 24h by node** (name, kWh, bar scaled to the largest node) and the 24-bar chart gains a `-24h · -12h · now` axis, a brighter newest bar, and a `Wh per output token` explanation on the Efficiency stat.
- **GPU processes + daily trend on the node console** — the top-5 VRAM consumers already in every snapshot (`metrics.gpu.processes`) render in CH·01 as name / pid / VRAM with a share-of-total bar; `LlmDailyChart` (14-day decode tok/s, previously implemented but never imported) renders in CH·02 under the serving log.
...
- **Models tab = NAS catalog only** — per the approved mockup: HF download form + Name/Runtime/Repository/Size/Delete table with a summary chip and stale/modelctl badges. The per-node matrix, node select, serving controls and active-jobs panel are removed from the tab.
- **Overview gauges grouped per device** — the GPU and CPU gauges now carry both readings of their device: headline value stays the temperature, the bar/foot show utilization (GPU foot adds SM clocks from the throttle probe, CPU foot adds package draw/tdp), and the sparkline trends usage. The standalone GPU `Usage` gauge is gone; when the remaining gauge count is odd the last cell spans the row.
- **The `/spark` console is four channels again — CH·03 Models is gone.** Engine launching moved into CH·02's always-present Serve script panel, so per-node serving no longer depends on modelctl enrollment. The per-node sync/push/delete UI is removed from the product (the global Models page was already NAS-catalog-only); Tests renumbers to CH·03 and Node to CH·04, and the node page drops its now-unused jobs/modelctl-release polling.
- **Install agent is truthful** — the CTA cluster is a tri-state driven by the actual transport: `Agent v<version>` (green, connected) / `Agent offline` (warning, enabled-but-SSH-fallback, with Install) / `Agent off` (muted, not enabled → point at Edit Spark). Installing now polls the job (1 s, 90 s cap) and reports *Installing… → Agent connected* only on the real hello, or the job's `lastError` on failure; the stale "watch the Jobs strip on the Models channel" hint is gone with that channel.

### Fixed
- **catalog.json lagged the store after every download** — the catalog snapshot only regenerated via the explicit ⟳ Refresh catalog job, so a just-finished download was live in `modelctl list` (both catalog tables) but absent from catalog.json (NasPage showed a green OK with a hours-old generation). `download` / `queue` / `nas-delete` job scripts now chain `modelctl catalog refresh` when their mutation succeeded; a refresh failure is logged without flipping the job's exit code.
- **Completed jobs kept a stale transient SSH error** — one failed status poll (node briefly unreachable / wedged ControlMaster) stored its message in the job record and the UI printed it next to `completed` forever; a successful poll now clears `lastError`. The same failure also surfaced as Node's raw `Command failed: sshpass -e ssh <full argv>` when ssh died with empty stderr — empty stderr now reports `timed out after <N>ms` (exec-timeout kill) or the bare exit code instead of dumping the whole command line.
...
- **`modelctl doctor --json` parsed as raw text** — modelctl emits a top-level *array* of audit items; the parser sliced only a `{…}` object, so every report degraded to `{raw}` and the UI showed JSON soup. The parser now accepts the array (canonical) and objects (defensive), and the NAS page summarizes items by real status vocabulary (`N checks · … · repair-active can fix X`).
- **False "modelctl update available"** — the chip compared versions as strings, so installed `v0.18.0` flagged GitHub's latest `v0.9.6` as an update. `versionIsNewer()` (numeric semver) gates the chip on the NAS page and Overview card.
- **Overview Status stat truncated** — the `decoding · 2 run · 2 wait` stat ellipsized on narrow cards (`0 wa…`); it now wraps like the other long stats and hovering shows the full text.
- **Queue jobs always `interrupted`** — `buildQueueScript` ended in `exit $code`, quitting the job shell before the wrapper could append the `__SPARKDASH_EXIT` trailer, so every successful queue job lost its exit code. Now `(exit $code)`.
- **`install-agent` reported success when the agent never started** — the bootstrap script's `__SPARKDASH_EXIT:0` terminalized the job as `completed` ~1 s in, while `expectAgentConnect` waited for a hello that never arrived: the UI said "Agent connected" and the chip stayed `Agent offline`. Exit 0 from the *script* is no longer completion; an install-agent job now completes only when the agent's WebSocket hello lands (`agentConnected`) or fails at the 60 s deadline (`failStaleHelloJobs`), with a non-zero script exit still failing fast. Caught from a real install on a host without passwordless sudo.
- **Agent bootstrap gave up on hosts without passwordless sudo** — the fallback printed manual instructions and exited 0, so nothing was ever started. It now installs a **systemd user unit** (`~/.config/systemd/user/spark-command-agent.service`, no `User=` line, `WantedBy=default.target`) whenever the SSH user has a user manager — the common case for DGX Spark / Ubuntu desktop logins — exporting `XDG_RUNTIME_DIR` (non-interactive SSH lacks it) and attempting `enable-linger` best-effort. Only when neither path exists does it report `__AGENT_UNIT__none`, and that now exits 4 so the job fails loudly with the manual steps instead of silently succeeding.
