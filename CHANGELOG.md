...
- **Prefill bench running chip (Tests channel)** — an in-flight prefill benchmark is reported on the TST·03 card of the node detail page, with progress, the size being measured and the throughput of the last finished level (`running · 1/2 · 8k · 745 tok/s`); hovering lists the phase message plus one line per measured level (`4k · 745.3 tok/s · TTFT 1.20s`). The action becomes **Open prefill bench**, since a second concurrent run is rejected with 409. Poll-based (1 s while running, 10 s idle), no server change.
### Changed
