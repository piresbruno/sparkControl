...
- **Prefill bench running chip (Tests channel)** — an in-flight prefill benchmark is reported on the TST·03 card (progress `running · 1/2 · 8k`, phase message on hover) and the action becomes **Open prefill bench**, since a second concurrent run is rejected with 409. Poll-based (1 s while running, 10 s idle), no server change.
### Changed
