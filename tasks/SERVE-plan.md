...
against a fake engine; also fixed a PRE-EXISTING port-trace bug). P5: modelctl proposal **filed** → [piresbruno/modelctl#7](https://github.com/piresbruno/modelctl/issues/7)
(ROOT/recipes/NAME/ co-distribution + cluster inventory --json); MiaAI `recipe.json` sidecar proposal **discarded** (user decision, 2026-09-15 — draft deleted, not filed).
README/CHANGELOG updated. Script-class unification (rev 3 §1.4 "revisited after P3") **implemented**:
launch + runs + logs of the script-class lane moved into the Serve section (CH·01 rows + launch panel,
`GET /api/serve/scripts` fan-out, run-port persistence, force/blocked parity); node page keeps engine
bays/WS truth/hero stop + deep-links only.
